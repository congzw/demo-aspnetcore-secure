namespace NbSites.Web.PermissionChecks.RoleBased
{
    ...
    public class RoleBasedPermissionRuleLogic : IRoleBasedPermissionRuleLogic
    {
        public PermissionCheckResult CheckRule(RoleBasedPermissionRule rule, PermissionCheckContext checkContext)
        {
            ...
            if (rule.NeedGuest())
            {
                return PermissionCheckResult.Allowed.WithMessage("访客规则 => 满足").WithData(rule);
            }

            var hasLogin = userContext.IsLogin();
            if (!hasLogin)
            {
                return PermissionCheckResult.Forbidden.WithMessage("需要登录 => 不满足").WithData(rule);
            }

            if (rule.NeedLogin())
            {
                return PermissionCheckResult.Allowed.WithMessage("需要登录 => 满足").WithData(rule);
            }
            ...
            {
                return PermissionCheckResult.Allowed.WithMessage(msg + " => 满足").WithData(rule);
            }

            return PermissionCheckResult.Forbidden.WithMessage(msg + " => 不满足").WithData(rule);
        }
    }
}